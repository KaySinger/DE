...
class APDSDE:
    ...
    # 线性种群缩减策略
    def _nonlinear_pop_size_reduction(self):

        return max(self.N_min, int(round(self.N_init - (self.N_init - self.N_min) * (self.num_evals / self.max_evals) ** 2)))
    ...
    def _calculate_SPG(self):
        """计算策略选择概率SP_G（公式17）"""
        return 1 / (1 + np.exp(1 - (self.num_evals / self.max_evals) ** 2))
    ...
    def mutantion(self, F, i, SPG, X_Amean):
        ...
        Fw = F * (0.7 + 0.7 * self.num_evals / self.max_evals)
    ...
    def optimize(self):
        ...
        while i < self.max_evals:
            max_gen += 1
            n = int(round(self.N_init - (self.N_init - self.N_min) * (self.num_evals / self.max_evals) ** 2))
            i += n
        print(max_gen)
        ...
        while self.num_evals < self.max_evals:
            ...
            for i in range(self.N_current):
                ...
                # current-to-pbest/1变异策略
                mutant = self.mutantion(F, i, SPG, X_Amean)
            ...
            # --- LPSR关键步骤 ---
            # 更新种群大小
            new_N = self._nonlinear_pop_size_reduction()
            # 选择适应度最好的个体保留
