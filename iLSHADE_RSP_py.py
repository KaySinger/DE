...
class iLSHADE_RSP:
    ...
    def _calculate_rank_probabilities(self, exclude_index=None):
        """计算基于排序的选择概率 (关键改进)"""
        # 按适应度排序（最小化问题，适应度值小的在前）
    ...
    def select_by_rank(self, exclude_index=None, n=1):
        """基于排序选择个体 (关键改进)"""
        probabilities = self._calculate_rank_probabilities(exclude_index)
    ...
    def mutant(self, F, i, p_best_indices, max_gen):
        ...
        # 基于排序选择pr1和pr2
        pr1_idx = self.select_by_rank(exclude_index=i, n=1)[0]
        r1 = self.pop[pr1_idx]
        # r2 概率性从种群或存档中选
        if len(self.archive) > 0:
            prob_from_pop = len(self.archive) / (len(self.archive) + self.N_current)
            if np.random.rand() < prob_from_pop:
                r2 = np.array(self.archive[np.random.randint(0, len(self.archive))])
            else:
                pr2_idx = self.select_by_rank(exclude_index=i, n=1)[0]
                r2 = self.pop[pr2_idx]
        else:
            # 没有存档就从种群选
            pr2_idx = self.select_by_rank(exclude_index=i, n=1)[0]
    ...
    def _cauchy_perturbation(self, target):
        """柯西扰动操作"""
        perturbation = cauchy.rvs(loc=target, scale=0.1)
    ...
    def cross(self, mutant, i, CR):
        if np.random.rand() <= self.p_j:
            ...
            cross_chorm = self._cauchy_perturbation(cross_chorm)
        else:
            cross_chorm = self.pop[i].copy()
    ...
    def optimize(self):
        ...
        while self.num_evals < self.max_evals:
            ...
            # current-to-pbest/1变异策略
            p = 0.085 * (1 + self.num_evals / self.max_evals)
